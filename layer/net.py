...
class net:
	...
	def predict(self, eval_data=None, model=None):
		...
		# TODO added this line - test and cont.
		self.dropout_keep_prob = self.session.graph.get_tensor_by_name("state/dropout_keep_prob:0")
	...
	# TODO added this lines - cont.
	def predict_sort(self, eval_data=None, model=None):
		if eval_data is None:
			eval_data = np.random.random(self.input_shape)
		self.dropout_keep_prob = self.session.graph.get_tensor_by_name("state/dropout_keep_prob:0")
		feed_dict = {self.x: [eval_data], self.dropout_keep_prob: 1.0, self.train_phase: False}
		result = self.session.run([self.output], feed_dict)
		best = np.argsort(result)
		items_result = {}
		rank = 0
